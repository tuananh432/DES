...
public class DES_Encryption {
    ...
    public String encrypt(String key, String plaintext) {
        ...
        // thêm 0 vào đầu xâu nếu độ dài message % 64 != 0
        ...
        // Mã hóa từng khối thành nhị phân
        String binCiphertext = "";
        for (int i = 0; i < binCiphertextBlocks.length; i++) {
            binCiphertext += binCiphertextBlocks[i];
        }

        // Destroy key schedule -- Xóa key
    ...
    public String decrypt(String key, String plaintext) {
        ...
        // Chia xâu bin thành các khối có độ dài = 64 
        String[] binPlaintextBlocks = new String[binPlaintext.length() / 64];
        int offset = 0;
        for (int i = 0; i < binPlaintextBlocks.length; i++) {
            binPlaintextBlocks[i] = binPlaintext.substring(offset, offset + 64);
    ...
    public String encryptBlock(String plaintextBlock) throws Exception {
        // yêu cầu độ dài 64bit
        int length = plaintextBlock.length();
        if (length != 64) {
            throw new RuntimeException("Độ dài khối không đủ 64bit!");
        }

        // hoán vị
        String out = "";
        for (int i = 0; i < IP.length; i++) {
            // thay thế kí tự đó trong mảng IP với vị trí i
            out = out + plaintextBlock.charAt(IP[i] - 1);
        }
        // chia ra làm 2 khối với độ dài 32bit
        ...
        Bit đầu tiên trong chuỗi giá trị 48 bit là bit số 32 của R, 
        bit thứ 2 là bit số 1 của R, bit thứ 3 là bit số 2 của R và bit cuối cùng là bit số 1 của R.
        */

        for (int i = 0; i < 16; i++) {

            // Thêm "0" vào đầu chuỗi nếu độ dài chuỗi < 48
    ...
    public String decryptBlock(String plaintextBlock) throws RuntimeException {
        int length = plaintextBlock.length();
        if (length != 64) {
            throw new RuntimeException("Độ dài khối không phải 64 bit!");
        }
    ...
    // Viết hàm hash dựa trên hàm hashCode() có sẵn
    ...
    public void buildKeySchedule(long key) {
        ...
        // chuyển đổi giá trị key thành chuỗi 64 bit nhị phân
        String binKey = Long.toBinaryString(key);

        // Thêm "0" vào trước nếu độ dài < 64
        while (binKey.length() < 64) {
            binKey = "0" + binKey;
        }

        // Trường hợp với khóa 56 bit
        String binKey_PC1 = "";

        /*
        Một key có 64 bit nhưng chỉ có 56 bit được sử dụng để thực hiện 
        tính toán giá trị khóa vòng. Key được chia làm 8 byte. Các bit 
        ở vị trí 8, 16, 32, 40, 48, 56 và 64 là các bit parity được sử 
        dụng để kiểm tra độ chính xác của key theo từng byte vì khi key 
        được phân phối trên đường truyền đến bộ mã hóa giải mã thì có 
        thể xảy ra lỗi. Parity được sử dụng là parity lẻ (odd parity).
        */
        ...
        // chia hoán vị thành 2 phần L và R | 56/2 = 28
        ...
        for (int i = 1; i < K.length; i++) {
            ...
            // Thêm "0" vào trước xâu nếu độ dài ko đủ 56 bit
